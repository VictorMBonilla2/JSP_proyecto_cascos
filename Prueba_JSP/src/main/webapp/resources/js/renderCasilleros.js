...
document.addEventListener("DOMContentLoaded", async ()=>{
    ...
    const fetchcolors = await fetch(`${host}/listaTipoReportes`)
    const colors = await fetchcolors.json();
    const sectoresArray  = agruparEspaciosPorSector(espacios);
    renderTabs(sectoresArray);
    function renderEspacioOcupado(espacio) {
        ...
        clone.querySelector('.report__img').addEventListener("click", ()=>{
            modalReportEspacio(espacio,colors)
        })
    ...
    function renderTabs(sectores) {
        ...
        sectores.forEach((sector, index) => {
            ...
            sector.espacios.forEach(espacio => {
                let espacioDiv;
                if (espacio.persona) {
                    espacioDiv = renderEspacioOcupado(espacio); // Usar tu función para renderizar un espacio ocupado
                } else {
...
function modalReportEspacio(Espacio,colores){
